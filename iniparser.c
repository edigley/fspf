...
/*
	$Id: iniparser.c,v 2.8 2002/06/06 08:08:54 ndevilla Exp $
	$Author: ndevilla $
	$Date: 2002/06/06 08:08:54 $
	$Revision: 2.8 $
*/
...
{
    return dictionary_get(d, strlwc(key));
}
...
)
{
    ...
    found = 0 ;
    lc_entry = strlwc(entry);
    hash = dictionary_hash(lc_entry);
    for (i=0 ; i<ini->n ; i++)
    {
        if (hash==ini->hash[i])
        {
            if (!strcmp(lc_entry, ini->key[i]))
            {
                found = 1 ;
                break ;
            }
